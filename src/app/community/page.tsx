...
import { Users, Plus, Search, MessageSquare, ArrowRight, Trophy } from 'lucide-react';
...
export default function CommunityPage() {
  ...
  const [newCircle, setNewCircle] = useState({ name: '', description: '', roadmapId: '' });
              ...
              myCircles.map((m, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-foreground">{m.circle.name}</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {roadmaps.map(r => (
                <span key={r.id} className="text-[10px] font-bold px-2 py-1 rounded-full bg-secondary text-muted-foreground cursor-pointer hover:text-primary transition-colors">
                  {r.title}
                </span>
              ))}
            </div>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-32 rounded-3xl bg-secondary animate-pulse" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground line-clamp-2">{circle.description || 'No description provided.'}</p>
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <Button variant="ghost" size="sm" onClick={() => setIsCreateOpen(false)}><X className="w-5 h-5" /></Button>
              </div>
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-muted-foreground uppercase">Circle Name *</label>
                  <input 
                    required 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-muted-foreground uppercase">Associated Roadmap</label>
                  <select 
                    className="w-full p-3 rounded-xl border border-border bg-background focus:ring-2 focus:ring-primary outline-none transition-all" 
                    value={newCircle.roadmapId}
                    onChange={e => setNewCircle({...newCircle, roadmapId: e.target.value})}
                  >
